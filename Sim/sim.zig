...
pub const Sim = struct {
    ...
    pub fn init(allocator: std.mem.Allocator, dt: f64) Self {
        ...
        return Self{ .allocator = allocator, .dt = dt, .sim_objs = std.ArrayList(SimObject).init(allocator), .state_vals = std.ArrayList(f64).init(allocator), .state_names = std.ArrayList([]const u8).init(allocator) };
    }
