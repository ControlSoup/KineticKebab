...
pub fn mat_mul(array_matrix_1: *ArrayMatrixf64, array_matrix_2: *ArrayMatrixf64) !*ArrayMatrixf64 {
    if (array_matrix_1.compute_height() != array_matrix_2.width) {
        std.log.err("Unable to perform matrix multiplication with size [{d},{d}] vs [{d},{d}]", .{ array_matrix_1.width, array_matrix_1.compute_height(), array_matrix_2.width, array_matrix_2.compute_height() });
    }
...
test "ArrayMatrix" {
    ...
    defer arena.deinit();
    var arr = [_]f64{ 1.0, 3.0, 5.0, 8.0, 5.0, 6.0, 2.0, 3.0, 6.0 };
    ...
    var arr5 = [_]f64{ 1.0, -1.0, 1.0, 2.0, 3.0, -1.0, 3.0, -2.0, -9.0 };
