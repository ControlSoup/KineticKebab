...
pub const Vec2 = struct {
    ...
    pub fn from_angle_rad(norm_: f64, angle_rad: f64) Vec2 {
        return Vec2.init(norm_ * std.math.cos(angle_rad), norm_ * std.math.sin(angle_rad));
    }
...
test "dot_product" {
    ...
    try std.testing.expectApproxEqRel(a.dot(b), 0.0, 1e-6);

    // a • a = ||a||^2
    try std.testing.expectApproxEqRel(a.dot(a), a.norm_pow2(), 1e-6);
    ...
    b = Vec2.init(6.0, 7.0);
    // a • a = ||a||^2
    try std.testing.expectApproxEqRel(a.dot(b), 20.0, 1e-6);
}
...
test "unit_circle" {
    ...
    // Unit circle Q1
    try std.testing.expectApproxEqRel(a.norm(), 1.0, 1e-6);
    try std.testing.expectApproxEqRel(a.i, @sqrt(2.0) / 2.0, 1e-6);
    try std.testing.expectApproxEqRel(a.j, @sqrt(2.0) / 2.0, 1e-6);
