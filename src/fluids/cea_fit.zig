...
pub const PropLookup = struct {
    ...
    pub fn mr_to_base_props(self: *Self, mr: f64) void {
        const high_idx = try sim.math.index_geql(f64, self.mr_range[0..], mr);
        const low_idx = try sim.math.index_leql(f64, self.mr_range[0..], mr);

        if (high_idx == low_idx) return [2]f64{ self.gamma_range[high_idx], self.sp_r_range[low_idx] };

        self.gamma = sim.math.simple_lerp(mr, self.mr_range[low_idx], self.mr_range[high_idx], self.gamma_range[low_idx], self.gamma_range[high_idx]);
        self.sp_r = sim.math.simple_lerp(mr, self.mr_range[low_idx], self.mr_range[high_idx], self.sp_r_range[low_idx], self.sp_r_range[high_idx]);
        self.temp = sim.math.simple_lerp(mr, self.mr_range[low_idx], self.mr_range[high_idx], self.temp_range[low_idx], self.temp_range[high_idx]);
    }
};
...
pub const UpwindedCombuster = struct {
    ...
    pub fn update_props(self: *Self, guesses: []f64) !void {
        ...
        self.prop_lookup.mr_to_base_props(self.mr);

        // Update base props and lookup new properties from new temp
        self.intrinsic.update_base(self.prop_lookup.gamma, self.prop_lookup.sp_r);
        self.intrinsic.update_from_pt(self.press, self.intrinsic.temp);
